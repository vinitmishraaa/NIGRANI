...
export default function SearchPanel({ apiBase, modelsReady, deviceId, onComplete }) {
  ...
  async function openCamera() {
    ...
    try {
      ...
      const message = error?.name === "NotAllowedError" ? "Camera permission was denied. Allow camera access in the browser site settings and try again." : error?.name === "NotFoundError" ? "No camera/webcam was found on this device." : `Could not open the camera: ${error?.message || "unknown error"}`;
      setCameraError(message); setCameraOpen(true);
    }
  }
  ...
  async function capturePhoto() {
    ...
    if (!video?.videoWidth || !video?.videoHeight) { setCameraError("Camera is still starting. Please wait a moment and try again."); return; }
    const canvas = document.createElement("canvas"); const maxDimension = 1600;
    const scale = Math.min(1, maxDimension / Math.max(video.videoWidth, video.videoHeight));
    canvas.width = Math.max(1, Math.round(video.videoWidth * scale)); canvas.height = Math.max(1, Math.round(video.videoHeight * scale));
    canvas.getContext("2d", { alpha: false }).drawImage(video, 0, 0, canvas.width, canvas.height);
    const blob = await canvasToBlob(canvas, 0.88);
    await handleFileObject(new File([blob], "camera-capture.jpg", { type: "image/jpeg" })); closeCamera();
  }
  async function handleFile(e) { const file = e.target.files?.[0]; if (!file) return; e.target.value = ""; await handleFileObject(file); }
  async function handleFileObject(file) {
    setKind(""); setResult(null);
    try {
      setStatus("Preparing photo…"); const compressed = await compressImage(file); const dataUrl = await fileToDataUrl(compressed);
      setPreview(dataUrl); setImageData(dataUrl); setImageTransform({ scale: 1, x: 0, y: 0 });
      setStatus("Photo ready. Drag to move and pinch with two fingers to zoom.");
    } catch (error) { setStatus(error.message || "Could not prepare the image."); setKind("error"); }
  }

  function clampTransform(scale, x, y) {
    const box = imageBoxRef.current, img = imgRef.current;
    if (!box || !img) return { scale, x, y };
    const rect = box.getBoundingClientRect(); const baseWidth = img.offsetWidth || rect.width; const baseHeight = img.offsetHeight || rect.height;
    if (scale <= 1) return { scale, x: 0, y: 0 };
    const maxX = Math.max(0, (baseWidth * scale - rect.width) / 2), maxY = Math.max(0, (baseHeight * scale - rect.height) / 2);
  ...
  function beginImageGesture(event) {
    ...
    pointersRef.current.set(event.pointerId, { clientX: event.clientX, clientY: event.clientY }); const points = [...pointersRef.current.values()];
    if (points.length === 1) gestureRef.current = { mode: "pan", startX: event.clientX, startY: event.clientY, originX: imageTransform.x, originY: imageTransform.y, pointerId: event.pointerId };
    else if (points.length === 2) gestureRef.current = { mode: "pinch", startDistance: distance(points[0], points[1]), startScale: imageTransform.scale, startMidpoint: midpoint(points[0], points[1]), originX: imageTransform.x, originY: imageTransform.y };
  }
  function moveImageGesture(event) {
    if (!pointersRef.current.has(event.pointerId)) return; event.preventDefault();
    pointersRef.current.set(event.pointerId, { clientX: event.clientX, clientY: event.clientY }); const points = [...pointersRef.current.values()]; const gesture = gestureRef.current; if (!gesture) return;
    if (points.length >= 2 && gesture.mode === "pinch") {
      const ratio = gesture.startDistance ? distance(points[0], points[1]) / gesture.startDistance : 1; const nextScale = Math.max(0.55, Math.min(3, gesture.startScale * ratio)); const center = midpoint(points[0], points[1]);
  ...
  function endImageGesture(event) {
    ...
    else if (pointersRef.current.size === 1) { const [pointerId, point] = [...pointersRef.current.entries()][0]; gestureRef.current = { mode: "pan", startX: point.clientX, startY: point.clientY, originX: imageTransform.x, originY: imageTransform.y, pointerId }; }
  }
  function resetImageView() { setImageTransform({ scale: 1, x: 0, y: 0 }); }

  async function renderAdjustedImage() {
    const box = imageBoxRef.current; const source = await loadImage(preview); if (!box) return { dataUrl: preview, image: source };
    const rect = box.getBoundingClientRect(); const outWidth = 1200; const outHeight = Math.max(1, Math.round(outWidth * rect.height / rect.width));
    const canvas = document.createElement("canvas"); canvas.width = outWidth; canvas.height = outHeight; const ctx = canvas.getContext("2d", { alpha: false });
    ctx.fillStyle = "#000"; ctx.fillRect(0, 0, outWidth, outHeight);
    const baseScale = Math.max(rect.width / source.naturalWidth, rect.height / source.naturalHeight); const totalScale = baseScale * imageTransform.scale;
    const drawWidth = source.naturalWidth * totalScale, drawHeight = source.naturalHeight * totalScale; const left = (rect.width - drawWidth) / 2 + imageTransform.x; const top = (rect.height - drawHeight) / 2 + imageTransform.y; const sx = outWidth / rect.width;
    ctx.drawImage(source, left * sx, top * sx, drawWidth * sx, drawHeight * sx);
    let blob = await canvasToBlob(canvas, 0.9); if (blob.size > 460 * 1024) blob = await canvasToBlob(canvas, 0.72);
    const dataUrl = await fileToDataUrl(new File([blob], "adjusted-search.jpg", { type: "image/jpeg" })); return { dataUrl, image: await loadImage(dataUrl) };
  }
